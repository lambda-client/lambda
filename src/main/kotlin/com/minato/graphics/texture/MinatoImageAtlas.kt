...
object MinatoImageAtlas {
    ...
    fun upload(image: BufferedImage, name: String? = null): ImageEntry? {
        ...
        val textureId = Identifier.of("minato", "uploaded/$cacheKey")
    ...
    fun removeUploaded(name: String) {
        uploadedTextureCache.remove(name)?.let { uploaded ->
            mc.textureManager.destroyTexture(Identifier.of("minato", "uploaded/$name"))
        }
    }
    ...
    fun clearCache() {
        ...
        uploadedTextureCache.values.forEach { uploaded ->
            mc.textureManager.destroyTexture(Identifier.of("minato", "uploaded/${uploaded.id}"))
        }
