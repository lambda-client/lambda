...
class RegionRenderer {
	...
	fun upload(collector: RegionVertexCollector) {
		...
		val faceRes = uploadBatch(result.faces, "Minato World Face Buffer", faceVertexBuffer)
		faceVertexBuffer = faceRes.buffer
		faceIndexCount = faceRes.indexCount

		val edgeRes = uploadBatch(result.edges, "Minato World Edge Buffer", edgeVertexBuffer)
		edgeVertexBuffer = edgeRes.buffer
		edgeIndexCount = edgeRes.indexCount

		val textRes = uploadBatch(result.text, "Minato World Text Buffer", textVertexBuffer)
		textVertexBuffer = textRes.buffer
		textIndexCount = textRes.indexCount

		val sFaceRes = uploadBatch(screenResult.faces, "Minato Screen Face Buffer", screenFaceVertexBuffer)
		screenFaceVertexBuffer = sFaceRes.buffer
		screenFaceIndexCount = sFaceRes.indexCount

		val sEdgeRes = uploadBatch(screenResult.edges, "Minato Screen Edge Buffer", screenEdgeVertexBuffer)
		screenEdgeVertexBuffer = sEdgeRes.buffer
		screenEdgeIndexCount = sEdgeRes.indexCount

		val sTextRes = uploadBatch(screenResult.text, "Minato Screen Text Buffer", screenTextVertexBuffer)
		screenTextVertexBuffer = sTextRes.buffer
		screenTextIndexCount = sTextRes.indexCount

		worldImageBatches = uploadTextureBatches(result.images, "Minato World Image Buffer", worldImageBatches)
		screenImageBatches = uploadTextureBatches(screenResult.images, "Minato Screen Image Buffer", screenImageBatches)
		modelBatches = uploadTextureBatches(result.models, "Minato World Model Buffer", modelBatches)
		screenModelBatches = uploadTextureBatches(screenResult.models, "Minato Screen Model Buffer", screenModelBatches)

		val oldOutlined = outlinedBatches
		outlinedBatches = result.outlined.mapValues { (id, out) ->
			val old = oldOutlined[id]
			OutlinedBatchResult(
				faces = uploadBatch(out.faces, "Minato Outlined Face Buffer ($id)", old?.faces?.buffer),
				edges = uploadBatch(out.edges, "Minato Outlined Edge Buffer ($id)", old?.edges?.buffer),
				text = uploadBatch(out.text, "Minato Outlined Text Buffer ($id)", old?.text?.buffer),
				models = uploadTextureBatches(out.models, "Minato Outlined Model Buffer ($id)", old?.models ?: emptyList()),
				images = uploadTextureBatches(out.images, "Minato Outlined Image Buffer ($id)", old?.images ?: emptyList())
			)
		}
