...
class RegionVertexCollector {
	...
	private fun buildEdges(): BuiltBatch? {
		...
		BufferAllocator(vertices.size * 48).use { allocator ->
			val builder = BufferBuilder(allocator, VertexFormat.DrawMode.QUADS, MinatoVertexFormats.POSITION_COLOR_NORMAL_LINE_WIDTH_DASH)
			vertices.forEach { v ->
				builder.vertex(v.x, v.y, v.z).color(v.r, v.g, v.b, v.a)
				builder.beginElement(MinatoVertexFormats.NORMAL_FLOAT).let { p ->
					if (p != -1L) {
						MemoryUtil.memPutFloat(p, v.nx); MemoryUtil.memPutFloat(p + 4, v.ny); MemoryUtil.memPutFloat(p + 8, v.nz)
					}
				}
				builder.beginElement(MinatoVertexFormats.LINE_WIDTH_FLOAT).let { p ->
					if (p != -1L) MemoryUtil.memPutFloat(p, v.lineWidth)
				}
				builder.beginElement(MinatoVertexFormats.DASH_ELEMENT).let { p ->
					if (p != -1L) {
	...
	private fun buildText(): BuiltBatch? {
		...
		BufferAllocator(vertices.size * 64).use { allocator ->
			val builder = BufferBuilder(allocator, VertexFormat.DrawMode.QUADS, MinatoVertexFormats.POSITION_TEXTURE_COLOR_ANCHOR_SDF)
			vertices.forEach { v ->
				builder.vertex(v.localX, v.localY, v.layerType.toFloat()).texture(v.u, v.v).color(v.r, v.g, v.b, v.a)
				builder.beginElement(MinatoVertexFormats.ANCHOR_ELEMENT).let { p ->
					if (p != -1L) {
						MemoryUtil.memPutFloat(p, v.anchorX); MemoryUtil.memPutFloat(p + 4, v.anchorY); MemoryUtil.memPutFloat(p + 8, v.anchorZ)
					}
				}
				builder.beginElement(MinatoVertexFormats.BILLBOARD_DATA_ELEMENT).let { p ->
					if (p != -1L) {
						MemoryUtil.memPutFloat(p, v.scale); MemoryUtil.memPutFloat(p + 4, v.billboardFlag)
					}
				}
				builder.beginElement(MinatoVertexFormats.SDF_STYLE_ELEMENT).let { p ->
					if (p != -1L) {
	...
	private fun buildScreenFaces(): BuiltBatch? {
		...
		BufferAllocator(vertices.size * 24).use { allocator ->
			val builder = BufferBuilder(allocator, VertexFormat.DrawMode.QUADS, MinatoVertexFormats.SCREEN_FACE_FORMAT)
			vertices.forEach { v -> 
				builder.vertex(v.x, v.y, 0f).color(v.r, v.g, v.b, v.a)
				builder.beginElement(MinatoVertexFormats.LAYER_ELEMENT).let { p ->
	...
	private fun buildScreenEdges(): BuiltBatch? {
		...
		BufferAllocator(vertices.size * 52).use { allocator ->
			val builder = BufferBuilder(allocator, VertexFormat.DrawMode.QUADS, MinatoVertexFormats.SCREEN_LINE_FORMAT)
			vertices.forEach { v ->
				builder.vertex(v.x, v.y, 0f).color(v.r, v.g, v.b, v.a)
				builder.beginElement(MinatoVertexFormats.DIRECTION_2D_ELEMENT).let { p ->
	...
	private fun buildScreenText(): BuiltBatch? {
		...
		var result: BuiltBatch? = null
		BufferAllocator(vertices.size * 48).use { allocator ->
			val builder = BufferBuilder(allocator, VertexFormat.DrawMode.QUADS, MinatoVertexFormats.SCREEN_TEXT_SDF_FORMAT)
			vertices.forEach { v ->
		...
		screenImageBatches.forEach { (batchKey, vertexDeque) ->
			...
			BufferAllocator(vertices.size * 44).use { allocator ->
				...
				vertices.forEach { v ->
					...
					val overlayPointer = builder.beginElement(MinatoVertexFormats.OVERLAY_UV_ELEMENT)
					...
					val layerPointer = builder.beginElement(MinatoVertexFormats.LAYER_ELEMENT)
					if (layerPointer != -1L) {
	...
	fun buildWorldImageBatches(): List<BuiltTextureBatch> {
		...
		worldImageBatches.forEach { (batchKey, vertexDeque) ->
			...
			BufferAllocator(vertices.size * 64).use { allocator ->
				val builder = BufferBuilder(allocator, VertexFormat.DrawMode.QUADS, MinatoVertexFormats.WORLD_IMAGE_FORMAT)
				vertices.forEach { v ->
					...
					builder.beginElement(MinatoVertexFormats.OVERLAY_UV_ELEMENT).let { p ->
	...
	fun buildModelBatches(): List<BuiltTextureBatch> {
		...
		worldModelBatches.forEach { (batchKey, vertexDeque) ->
			...
			BufferAllocator(vertices.size * 88).use { allocator ->
				val builder = BufferBuilder(allocator, VertexFormat.DrawMode.QUADS, MinatoVertexFormats.WORLD_MODEL_FORMAT)
				vertices.forEach { v ->
					...
					builder.beginElement(MinatoVertexFormats.LIGHT_DIR_ELEMENT).let { p ->
						if (p != -1L) { MemoryUtil.memPutFloat(p, v.lx); MemoryUtil.memPutFloat(p + 4, v.ly); MemoryUtil.memPutFloat(p + 8, v.lz) }
					}
					builder.beginElement(MinatoVertexFormats.LIGHT_1_DIR_ELEMENT).let { p ->
					...
					builder.beginElement(MinatoVertexFormats.EDGE_DATA_ELEMENT).let { p ->
