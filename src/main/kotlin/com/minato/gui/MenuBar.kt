...
import com.minato.Minato.REPO_URL
...
import com.minato.gui.DearImGui.EXTERNAL_LINK
...
import com.minato.util.Diagnostics.gatherDiagnostics
import com.minato.util.FolderRegistry
import com.minato.util.FolderRegistry.minecraft
...
object MenuBar {
    ...
    var newConfigName = ""
    val headerLogo = upload("textures/minato_text_color.png")
    val minatoLogo = upload("textures/minato.png")
    ...
    private fun ImGuiBuilder.minatoMenu() {
        ...
        val pad = 2f
        val minatoIconSize = (headerH - pad * 2f).coerceAtLeast(1f)
        val iconX = itemRectMinX + (headerW - minatoIconSize) * 0.5f
        val iconY = itemRectMinY + (headerH - minatoIconSize) * 0.5f

        foregroundDrawList.addImage(
            minatoLogo.id.toLong(),
            iconX, iconY,
            iconX + minatoIconSize, iconY + minatoIconSize
        )
        ImGui.popStyleColor()
    ...
    private fun ImGuiBuilder.buildMinatoMenu() {
        menu("Save Config...") {
        ...
        menu("Settings") {
            ...
            menu("Minato API Settings") {
                buildConfigSettingsContext(MinatoAPI)
            }
        }
        separator()
        menu("Open Folder") {
            menuItem("Open Minato Folder") {
                Util.getOperatingSystem().open(FolderRegistry.minato)
            }
        ...
        separator()
        menuItem("Close GUI", "Esc") { MinatoScreen.close() }
    ...
    private fun ImGuiBuilder.aboutPopup() {
        popupModal("About Minato", ImGuiWindowFlags.AlwaysAutoResize or ImGuiWindowFlags.NoTitleBar) {
            imageHorizontallyCentered(headerLogo.id.toLong(), 553f, 200f)
            group {
                text("Version: ${Minato.VERSION}")
                if (Minato.isDebug) text("Development Environment")
            ...
            separator()
            text("Authors")
            FabricLoader.getInstance().getModContainer("minato")
                .orElseThrow { IllegalStateException("Could not find Minato mod container!") }
                .metadata
                .authors
                .forEach { author ->
            ...
            withStyleColor(ImGuiCol.Button, 0x00000000) {
                withStyleColor(ImGuiCol.ButtonHovered, 0x22FFFFFF) {
                    withStyleColor(ImGuiCol.ButtonActive, 0x44FFFFFF) {
                        ...
                        minatoTooltip("Open GitHub Repository $EXTERNAL_LINK")
                        if (clicked) {
