...
import com.minato.config.settings.complex.KeybindSetting.Companion.onPressUnsafe
...
import com.minato.gui.MenuBar
import com.minato.gui.MenuBar.buildMenuBar
import com.minato.gui.OverlayBackgroundScreen
import com.minato.gui.components.QuickSearch.renderQuickSearch
...
import com.minato.module.tag.ModuleTag.Companion.shownTags
...
object ClickGuiLayout : Loadable, Config(
	...
	@Tab(GENERAL_TAB) val setMinatoWindowIcon by setting("Set Minato Window Icon", true)
		.onValueChange { _, to ->
			if (to) setMinatoWindowIcon()
			else {
	...
	@JvmStatic
	@Tab(GENERAL_TAB) val setMinatoWindowTitle by setting("Set Minato Window Title", true).onValueChange { _, _ -> mc.updateWindowTitle() }
	@Tab(GENERAL_TAB) val minatoTitleAppendixName by setting("Append Username", true) { setMinatoWindowTitle }.onValueChange { _, _ -> mc.updateWindowTitle() }
	...
	fun toggle() {
		if (open) {
			// MinatoScreen.close() restores the background screen and triggers
			// removed() -> close(), which flips `open` off and plays the sound.
			MinatoScreen.close()
		} else {
			val current = mc.currentScreen
			if (current.hasInput) return
			if (Client.clientSounds) MinatoSound.ModuleOn.play()
			MinatoScreen.parentScreen = if (current is MinatoScreen) null else current
	...
	fun close() {
		if (Client.clientSounds) MinatoSound.ModuleOff.play()
		open = false
	}
