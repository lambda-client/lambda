...
object SwingAnimation : Module(
    ...
    init {
        ...
        immediateRenderer("SwingAnimation 3D") {
            ...
            // Sprint 3D effects (dust + afterimages)
            if (sprintFootstepDust) {
                FootstepDustLayer.run { this@immediateRenderer.render(cameraPos, SprintTrailManager.speedIntensity) }
            }
            if (sprintDashAfterimage) {
                DashAfterimageLayer.run { this@immediateRenderer.render(cameraPos, SprintTrailManager.speedIntensity) }
            }
            ...
            synchronized(killEffects) {
                killEffects.forEach { seq ->
                    if (killLightning) {
                        ...
                        val coreInt = if (killUseGuiTheme) com.minato.gui.components.ClickGuiLayout.primaryColor.rgb else killCoreColor.rgb
                        val glowInt = if (killUseGuiTheme) com.minato.gui.components.ClickGuiLayout.primaryColor.rgb else killGlowColor.rgb
        ...
        listen<HudRenderEvent> { event ->
            ...
            if (isEnabled) {
                if (sprintSpeedLines) SpeedLineLayer.render(window.width, window.height, drawContext)
