...
object HoleFiller :
        ...
        safeListener<TickEvent.ClientTickEvent> {
            defaultScope.launch {
                for (x in -range..range) for (y in -range..range) for (z in -range..range) {
                    closestPlayer = getClosestPlayer() ?: continue
    ...
    private fun SafeClientEvent.place(closestPlayer: EntityPlayer){
        ...
        if (player.distanceTo(closestPlayer.position) <= playerRange &&
            ...
            if (rotate) (rotationPacket()).let { PacketManager.postPacket(it) }
            placePacket?.let { PacketManager.postPacket(it) }
            resetHotbar()
            placePacket = null
    ...
    private fun SafeClientEvent.getClosestPlayer(): EntityPlayer? {
    ...
    private fun SafeClientEvent.rotationPacket(): Packet<*> {
        val rotation = if(legitPlace) getLegitRotations(vec) else getRotationTo(vec).toVec2d()
        return CPacketPlayer.Rotation(rotation.x.toFloat(), rotation.y.toFloat(), player.onGround)
    }
    ...
    private fun getLegitRotations(vec: Vec3d): Vec2d {
        ...
        return Vec2d(mc.player.rotationYaw + MathHelper.wrapDegrees(yaw - mc.player.rotationYaw).toDouble(), mc.player.rotationPitch + MathHelper.wrapDegrees(pitch - mc.player.rotationPitch).toDouble())
    }
