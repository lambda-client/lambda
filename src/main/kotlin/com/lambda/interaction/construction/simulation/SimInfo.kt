...
import com.lambda.interaction.construction.simulation.BasicChecker.hasBasicRequirements
...
/**
 * Creates a [SimInfo], checks its basic requirements, and runs the [SimInfo.sim] block.
 */
@SimDsl
...
) = getTypedInfo(pos, state, targetState, pov, Stack(), concurrentResults)
	.takeIf { it?.hasBasicRequirements() == true }?.sim()

/**
 * Creates a new [SimInfo] using the current [SimInfo]'s [SimInfo.dependencyStack] and [SimInfo.concurrentResults],
 * checks its basic requirements, and runs the [SimInfo.sim] block. As simulations tend to make use of
