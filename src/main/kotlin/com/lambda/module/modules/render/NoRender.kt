...
object NoRender : Module(
	...
	@JvmStatic
	val noBlindness by setting("No Blindness", true).group(Group.Effect)
	@JvmStatic
	val noDarkness by setting("No Darkness", true).group(Group.Effect)
	@JvmStatic
	val noNausea by setting("No Nausea", true).group(Group.Effect)

	@JvmStatic
	val noFireOverlay by setting("No Fire Overlay", false).group(Group.Hud)
	@JvmStatic
	val fireOverlayYOffset by setting("Fire Overlay Y Offset", 0.0, -0.4..0.4, 0.02) { !noFireOverlay }.group(Group.Hud)
	@JvmStatic
	val noPortalOverlay by setting("No Portal Overlay", true).group(Group.Hud)
	@JvmStatic
	val noFluidOverlay by setting("No Fluid Overlay", true).group(Group.Hud)
	@JvmStatic
	val noPowderedSnowOverlay by setting("No Powdered Snow Overlay", true).group(Group.Hud)
	@JvmStatic
	val noInWall by setting("No In Wall Overlay", true).group(Group.Hud)
	@JvmStatic
	val noPumpkinOverlay by setting("No Pumpkin Overlay", true).group(Group.Hud)
	@JvmStatic
	val noVignette by setting("No Vignette", true).group(Group.Hud)
	@JvmStatic
	val noChatVerificationToast by setting("No Chat Verification Toast", true).group(Group.Hud)
	@JvmStatic
	val noSpyglassOverlay by setting("No Spyglass Overlay", false).group(Group.Hud)
	@JvmStatic
	val noGuiShadow by setting("No Gui Shadow", false).group(Group.Hud)
	@JvmStatic
	val noFloatingItemAnimation by setting("No Floating Item Animation", false, "Disables floating item animations, typically used when a totem pops").group(Group.Hud)
	@JvmStatic
	val noCrosshair by setting("No Crosshair", false).group(Group.Hud)
	@JvmStatic
	val noBossBar by setting("No Boss Bar", false).group(Group.Hud)
	@JvmStatic
	val noScoreBoard by setting("No Score Board", false).group(Group.Hud)
	@JvmStatic
	val noStatusEffects by setting("No Status Effects", false).group(Group.Hud)

	@JvmStatic
	val noArmor by setting("No Armor", false).group(Group.Entity)
	@JvmStatic
	val includeNoOtherHeadItems by setting("Include No Other Head Items", false) { noArmor }.group(Group.Entity)
	@JvmStatic
	val noElytra by setting("No Elytra", false).group(Group.Entity)
	@JvmStatic
	val noInvisibility by setting("No Invisibility", true).group(Group.Entity)
	@JvmStatic
	val noGlow by setting("No Glow", false).group(Group.Entity)
	@JvmStatic
	val noNametags by setting("No Nametags", false).group(Group.Entity)
	...
	@JvmStatic
	val noTerrainFog by setting("No Terrain Fog", false).group(Group.World)
	@JvmStatic
	val noSignText by setting("No Sign Text", false).group(Group.World)
	@JvmStatic
	val noWorldBorder by setting("No World Border", false).group(Group.World)
	@JvmStatic
	val noEnchantingTableBook by setting("No Enchanting Table Book", false).group(Group.World)
	...
	@JvmStatic
	val noBeaconBeams by setting("No Beacon Beams", false).group(Group.World)
	@JvmStatic
	val noSpawnerMob by setting("No Spawner Mob", false).group(Group.World)
