...
object InventoryCleaner : Module(
	...
	private val itemsCanTrash by setting(
		"Items Can Trash", setOf<Item>(Items.NETHERRACK, Items.COBBLESTONE), ITEM.toSet(),
		description = """A list of items that the module can trash when trying to make space in the inventory.
		""".trimIndent()
	)

	private val dropOnContainerClick by setting(
		"Drop On Container Click", true,
		description = """If enabled, shift + left clicking an item in a container will drop a trash item out of the inventory if the player inventory is full.
		""".trimIndent()
	)
	private val dropToPickup by setting(
		...
		Items to pickup can be configured in the 'Items to pickup' setting. Trash items to drop can be configured in the 'Items can trash' setting.
		""".trimIndent()
	)
