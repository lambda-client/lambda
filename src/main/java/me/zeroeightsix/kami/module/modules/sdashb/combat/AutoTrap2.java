...
public class AutoTrap2 extends Module
{
    ...
    public void onUpdate() {
        if (AutoTrap2.mc.player == null || ModuleManager.isModuleEnabled("Freecam")) {
            return;
        }
        ...
        while (blocksPlaced < this.blockPerTick.getValue()) {
            ...
            for (final Entity entity : AutoTrap2.mc.world.getEntitiesWithinAABBExcludingEntity((Entity)null, new AxisAlignedBB(targetPos))) {
    ...
    private boolean placeBlock(final BlockPos pos) {
        if (!AutoTrap2.mc.world.getBlockState(pos).getMaterial().isReplaceable()) {
            return false;
        }
        ...
        for (final EnumFacing side : EnumFacing.values()) {
            ...
            if (AutoTrap2.mc.world.getBlockState(neighbor).getBlock().canCollideCheck(AutoTrap2.mc.world.getBlockState(neighbor), false)) {
                ...
                if (eyesPos.distanceTo(hitVec) <= this.range.getValue()) {
                    ...
                    final Block neighborPos = AutoTrap2.mc.world.getBlockState(neighbor).getBlock();
                    ...
                    AutoTrap2.mc.playerController.processRightClickBlock(AutoTrap2.mc.player, AutoTrap2.mc.world, neighbor, side2, hitVec, EnumHand.MAIN_HAND);
