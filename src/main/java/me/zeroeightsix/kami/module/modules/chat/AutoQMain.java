...
public class AutoQMain extends Module {
    ...
    public void onUpdate() {
        ...
        if (Minecraft.getMinecraft().getCurrentServerData() == null) {
            Command.sendWarningMessage(getChatName() + "&l&6Warning: &r&6You are on singleplayer");
            return;
        }
        if (!Minecraft.getMinecraft().getCurrentServerData().serverIP.equalsIgnoreCase("2b2t.org") && debugWarn.getValue()) {
            Command.sendWarningMessage(getChatName() + "&l&6Warning: &r&6You are not connected to 2b2t.org");
        }
        if (mc.player.dimension != 1 && endDi.getValue()) {
            Command.sendWarningMessage(getChatName() + "&l&6Warning: &r&6You are not in the end. Not running &b/queue main&7.");
