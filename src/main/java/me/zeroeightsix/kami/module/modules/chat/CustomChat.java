...
public class CustomChat extends Module {
    ...
    public Setting<String> customText = register(Settings.stringBuilder("Custom Text").withValue("unchanged").withConsumer((old, value) -> {
    }).build());
    ...
    private String getFull(DecoMode d) {
        switch (d) {
            case NONE:
                return " " + getText(textMode.getValue());
            case CLASSIC:
                return " \u00ab " + getText(textMode.getValue()) + " \u00bb";
            case SEPARATOR:
                return " " + separator + " " + getText(textMode.getValue());
            default:
                return "";
        }
    }
