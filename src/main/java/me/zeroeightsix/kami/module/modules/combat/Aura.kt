...
class Aura : Module() {
    ...
    private val autoSpamDelay = register(Settings.booleanBuilder("AutoSpamDelay").withValue(true).withVisibility { delayMode.value == WaitMode.SPAM }.build())
    private val waitTick = register(Settings.doubleBuilder("SpamDelay").withMinimum(0.1).withValue(2.0).withMaximum(20.0).withVisibility { !autoSpamDelay.value && delayMode.value == WaitMode.SPAM }.build())
    ...
    private val passive = register(Settings.booleanBuilder("PassiveMobs").withValue(false).withVisibility { attackMobs.value }.build())
    private val neutral = register(Settings.booleanBuilder("NeutralMobs").withValue(false).withVisibility { attackMobs.value }.build())
    private val hostile = register(Settings.booleanBuilder("HostileMobs").withValue(false).withVisibility { attackMobs.value }.build())
    private val hitRange = register(Settings.d("HitRange", 5.5))
